...
namespace CMDInjectorHelper
{
    ...
    {
        public static bool installOnProcess { get; set; }
        ...
        public static async Task<StorageFolder> GetBackupFolder()
        {
            ...
            var result = await Helper.DisplayMessage2("Please select a parent folder to save all the PacMan packages backup.", Helper.SoundHelper.Sound.Alert, "", "Browse");
            if (result == 0)
            {
        ...
        public static List<string> ExtractAppxFromBundle(string zipFilePath, string destinationFolder)
        {
            ...
            try
            {
                ...
                {
                    if (Path.GetExtension(zipEntry.Name).ToLower() == ".appx")
                    {
                        packs.Add(zipEntry.Name);
        ...
        public static bool CheckFileExistInPackage(string zipFilePath, string extension)
        {
            ZipFile file = null;
            try
            {
                ...
                foreach (ZipEntry zipEntry in file)
                {
                    if (Path.GetExtension(zipEntry.Name).ToLower() == $".{extension}")
                        return true;
                }
            }
            finally
