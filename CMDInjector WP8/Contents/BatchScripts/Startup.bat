...
for /f "skip=2 tokens=1,2,3" %%a in ('reg query "HKLM\Software\Microsoft\SecurityManager\CapabilityClasses" /f ID_CAP* ^| findstr /iv /c:CAPABILITY_CLASS_THIRD_PARTY_APPLICATIONS') do if %%c neq search: reg add "HKLM\software\Microsoft\SecurityManager\CapabilityClasses" /v %%a /t %%b /d %%c\0CAPABILITY_CLASS_THIRD_PARTY_APPLICATIONS /f
